...
class NormalTransientPopup(wx.PopupTransientWindow):
    ...
    def __init__(self, parent, style, message, backgrdcol, foregrdcol):
        ...
        stext = wx.StaticText(panel, -1, message)
        stext.SetForegroundColour(wx.Colour(foregrdcol))
        sizer = wx.BoxSizer(wx.VERTICAL)
        sizer.Add(stext, 0, wx.ALL, 5)
...
class PopupDialog(wx.Dialog):
    ...
    Usage:
            loadDlg = PopupDialog(None, ("FFcuesplitter-GUI - Loading"),
                                  ("\nWait....\nwork in progress.\n")
                                  )
            loadDlg.ShowModal()
            loadDlg.Destroy()
    ...
    def __init__(self, parent, title, msg):
        ...
        if hasattr(wx, 'ActivityIndicator'):  # wxPython => 4.1
            ...
            actind = wx.ActivityIndicator(self)
            actind.Start()
            box2.Add(actind, 0, wx.LEFT
